...
def pred(validate_seq, validate_cond, modules, args, device):
    ...
    output_seq = [validate_seq[0]]
    # Create h_seq for each validate image.
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        if i < args.n_past:
            ...
            output_seq.append(validate_seq[i])
        else:
            z_t = torch.randn(args.batch_size, args.z_dim).cuda()
            g_t = modules['frame_predictor'](torch.cat([validate_cond[i - 1], h, z_t], 1)).detach()
            x_pred = modules['decoder']([g_t, skip]).detach()
            h_seq[i] = modules['encoder'](x_pred)
            output_seq.append(x_pred)
...
def plot_GIF(validate_seq, validate_cond, modules, epoch, args):
    n_sample = 5
    output_seq = [[] for i in range(n_sample)]
    # ground truth.
    gt_seq = [validate_seq[i] for i in range(len(validate_seq))]

    h_seq = [modules['encoder'](validate_seq[i]) for i in range(args.n_past + args.n_future)]
    for num_of_sample in range(n_sample):
        modules['frame_predictor'].hidden = modules['frame_predictor'].init_hidden()
        modules['posterior'].hidden = modules['posterior'].init_hidden()

        output_seq[num_of_sample].append(validate_seq[0])
        for i in range(1, args.n_past + args.n_future):
            # TODO
            if args.last_frame_skip or i < args.n_past:
                h, skip = h_seq[i - 1]
            ...
            if i < args.n_past:
                ...
                modules['frame_predictor'](torch.cat([validate_cond[i - 1], h, z_t], 1))
                x_in = validate_seq[i]
                output_seq[num_of_sample].append(x_in)
            else:
                g_t = modules['frame_predictor'](torch.cat([validate_cond[i - 1], h, z_t], 1)).detach()
                x_pred = modules['decoder']([g_t, skip]).detach()
                h_seq[i] = modules['encoder'](x_pred)
                output_seq[num_of_sample].append(x_pred)
    ...
    for i in range(n_row):
        ...
        for t in range(args.n_past + args.n_future):
            row = [gt_seq[t][i]]
            for num_of_sample in range(n_sample):
                row.append(output_seq[num_of_sample][t][i])
            gifs[t].append(row)

    file_name = '%s/gen/sample_%d.gif' % (args.log_dir, epoch)
    save_gif(file_name, gifs)
...
def plot_pred(validate_seq, validate_cond, modules, epoch, args):
    ...
    for i in range(n_row):
        row = []
        for t in range(args.n_past + args.n_future):
            row.append(output_seq[t][i])
        to_plot.append(row)
    file_name = '%s/gen/pred_%d.png' % (args.log_dir, epoch)
    save_tensors_image(file_name, to_plot)
